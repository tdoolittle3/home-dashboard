        ...
        {readings.length > 0 ? (
          <ul className="system__tiles">
            {readings.map((ref) => {
              ...
              return (
                <li key={ref.entity_id} className={`system__tile system__tile--${level}`}>
                  <span className="system__tile-label">{labelFor(ref, entity)}</span>
                  <span className="system__tile-value">
                    {value === null ? formatState(entity) : formatNumber(value)}
                    {unit ? <span className="system__tile-unit">{unit}</span> : null}
                  </span>
                  {kind === 'percent' ? (
                    <span className="system__tile-bar" aria-hidden="true">
                      <span
                        className="system__tile-fill"
